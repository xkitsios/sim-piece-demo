...
class Field(object):
    ...
    def copy(field):
        ...
        if output.get_data_type() is not None:
            if output.get_data_type() == TSDataType.BOOLEAN:
                ...
                raise Exception(
                    "unsupported dataset type {}".format(output.get_data_type())
                )
        return output
    ...
    def get_string_value(self):
        if self.__data_type is None:
            ...
            raise Exception("unsupported dataset type {}".format(self.__data_type))
    ...
    def get_object_value(self, data_type):
        ...
        if self.__data_type is None:
            ...
            raise Exception("unsupported dataset type {}".format(data_type))

    @staticmethod
    def get_field(value, data_type):
        """
        :param value: field value corresponding to the dataset type
        :param data_type: TSDataType
        """
        if value is None:
            return None
