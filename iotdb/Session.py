...
class Session(object):
    ...
    def create_time_series(
        ...
        :param data_type: TSDataType, dataset type for this time series
    ...
    def create_aligned_time_series(
        ...
        :param data_type_lst: List of TSDataType, dataset types for time series
    ...
    def delete_time_series(self, paths_list):
        """
        delete multiple time series, including dataset and schema
    ...
    def delete_data(self, paths_list, end_time):
        """
        delete all dataset <= end_time in multiple time series
        :param paths_list: time series list that the dataset in.
        :param end_time: dataset with time stamp less than or equal to time will be deleted.
        """
        request = TSDeleteDataReq(self.__session_id, paths_list, -9223372036854775808, end_time)
        try:
            status = self.__client.deleteData(request)
            logger.debug(
                "delete dataset from {}, message: {}".format(paths_list, status.message)
            )
        except TTransport.TException as e:
            logger.exception("dataset deletion fails because: ", e)

    def delete_data_in_range(self, paths_list, start_time, end_time):
        """
        delete dataset >= start_time and dataset <= end_time in multiple timeseries
        :param paths_list: time series list that the dataset in.
    ...
    def insert_record(self, device_id, timestamp, measurements, data_types, values):
        ...
        :param timestamp: Integer, indicate the timestamp of the row of dataset
        :param measurements: List of String, sensor names
        :param data_types: List of TSDataType, indicate the dataset type for each sensor
    ...
    def insert_records(
        ...
        """
        insert multiple rows of dataset, records are independent to each other, in other words, there's no relationship
        ...
        :param types_lst: 2-D List of TSDataType, each element of outer list indicates sensor dataset types of a device
    ...
    def insert_aligned_records(
        ...
        """
        insert multiple aligned rows of dataset, records are independent to each other, in other words, there's no relationship
        between those records
        :param device_ids: List of String, time series paths for device
        :param times: List of Integer, timestamps for records
        :param measurements_lst: 2-D List of String, each element of outer list indicates measurements of a device
    ...
    def test_insert_record(
        ...
        """
        this method NOT insert dataset into database and the server just return after accept the request, this method
    ...
    def gen_insert_record_req(
        ...
        if (len(values) != len(data_types)) or (len(values) != len(measurements)):
            raise RuntimeError(
                "length of dataset types does not equal to length of values!"
            )
    ...
    def test_insert_tablet(self, tablet):
        """
         this method NOT insert dataset into database and the server just return after accept the request, this method
         should be used to test other time cost in client
        :param tablet: a tablet of dataset
        """
    ...
    def value_to_bytes(data_types, values):
        ...
        for data_type, value in zip(data_types, values):
            if data_type == TSDataType.BOOLEAN.value:
                ...
                raise RuntimeError("Unsupported dataset type:" + str(data_type))
